...
async function handleRebuildIndexes(event, log, logs) {
  ...
  try {
    const listRes = await s3.send(
      new ListObjectsV2Command({ Bucket: BUCKET, Prefix: "users/" })
    );
    const userFolders = new Set();

    for (const obj of listRes.Contents || []) {
      const match = obj.Key.match(/^users\/([^/]+)\/account\.json$/);
      if (match) userFolders.add(match[1]);
    }

    const indexWrites = [];
    for (const uid of userFolders) {
      const accountKey = `users/${uid}/account.json`;
      try {
        const res = await s3.send(
          new GetObjectCommand({ Bucket: BUCKET, Key: accountKey })
        );
        const account = JSON.parse(await res.Body.transformToString());
        indexWrites.push(
          s3.send(
            new PutObjectCommand({
              Bucket: BUCKET,
              Key: `users/by-username/${account.username.toLowerCase()}.json`,
          ...
          s3.send(
            new PutObjectCommand({
              Bucket: BUCKET,
              Key: `users/by-email/${account.email.toLowerCase()}.json`,
              Body: JSON.stringify({ userId: uid }),
              ContentType: "application/json",
            })
          )
        ...
        log(`⚠️ Failed to process user ${uid}:`, err.message);
      }
    }
    ...
    log("❌ Failed to rebuild indexes:", err.message);
    return jsonResponse(
      500,
      "ERR_REBUILD_FAILED",
